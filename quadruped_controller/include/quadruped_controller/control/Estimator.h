...
public:
  KalmanFilterEstimate(CtrlInterfaces &ctrl_interfaces,
                       CtrlComponent &ctrl_component);

  ~KalmanFilterEstimate() = default;
  /// 估计的身体位置（世界系）
  Vec3 getPosition() { return x_hat_.segment(0, 3); }

  /// 估计的身体速度（世界系）
  Vec3 getVelocity() { return x_hat_.segment(3, 3); }
  ...
  RotMat getRotation() { return rotation_; }

  Vec3 getGyro() { return gyro_; }

  Vec3 getGyroGlobal() const { return rotation_ * gyro_; }

  double getYaw() const;

  double getDYaw() const { return getGyroGlobal()(2); }
