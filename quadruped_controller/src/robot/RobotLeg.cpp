...
RobotLeg::RobotLeg(const pinocchio::Model &model, pinocchio::Data &data,
                   const std::vector<pinocchio::JointIndex> &joint_ids,
                   pinocchio::FrameIndex foot_frame_id)
    : model_(model), data_(data), joint_ids_(joint_ids),
      foot_frame_id_(foot_frame_id) {}
...
Eigen::MatrixXd RobotLeg::calcJaco(const Eigen::VectorXd &q_full) {
  ...
  pinocchio::getFrameJacobian(model_, data_, foot_frame_id_,
                              pinocchio::LOCAL_WORLD_ALIGNED, J_6xN);
  return J_6xN.topRows(3);
}

Eigen::VectorXd RobotLeg::calcQ(const SE3 &target_pose,
...
Eigen::VectorXd RobotLeg::calcTorque(const Eigen::VectorXd &q_full,
                                     const Vec3 &foot_force) {
