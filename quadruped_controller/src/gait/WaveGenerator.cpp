...
WaveGenerator::WaveGenerator(const double period, const double st_ratio,
                             const Vec4 &bias) {
...
auto WaveGenerator::update() -> void {
  ...
  if (status_ != status_past_) {
    ...
    if (status_ == WaveStatus::STANCE_ALL &&
        status_past_ == WaveStatus::SWING_ALL) {
      contact_past_.setOnes();
    } else if (status_ == WaveStatus::SWING_ALL &&
...
void WaveGenerator::calcWave(Vec4 &phase, VecInt4 &contact,
                             const WaveStatus status) {
  switch (status) {
  case WaveStatus::WAVE_ALL: {
    const double past_t =
        static_cast<double>(getSystemTime() - start_t_) * 1e-6;
    for (int i(0); i < 4; ++i) {
      normal_t_(i) =
