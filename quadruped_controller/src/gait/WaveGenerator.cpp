...
WaveGenerator::WaveGenerator(const double period, const double st_ratio, const Vec4 &bias)
{
    ...
    {
        std::cerr << "[ERROR] The stancePhaseRatio of WaveGenerator should between (0, 1)" << std::endl;
        exit(-1);
    }
    ...
    for (int i(0); i < bias_.rows(); i++)
    {
        ...
        {
            std::cerr << "[ERROR] The bias of WaveGenerator should between [0, 1]" << std::endl;
            exit(-1);
        }
    }
    restart(status_);
...
void WaveGenerator::calcWave(Vec4 &phase, VecInt4 &contact, const WaveStatus status)
{
    ...
    case WaveStatus::WAVE_ALL: {
        ...
        for (int i(0); i < 4; ++i)
        {
            normal_t_(i) = fmod(past_t + period_ - period_ * bias_(i), period_) / period_;
