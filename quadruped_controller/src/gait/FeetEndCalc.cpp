...
Vec3 FeetEndCalc::calcFootPos(int index, Vec2 vxy_goal_global,
                              double d_yaw_global, double phase) {
  Vec3 body_vel_global = estimator_->getVelocity();
  Vec3 next_step;

  next_step(0) = body_vel_global(0) * (1 - phase) * t_swing_ +
                 body_vel_global(0) * t_swing_ / 2 +
  ...
  const double next_yaw = d_yaw * (1 - phase) * t_swing_ +
                          d_yaw * t_stance_ / 2 +
