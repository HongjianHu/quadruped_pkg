...
StateFixedStand::StateFixedStand(CtrlInterfaces &ctrl_interfaces,
                                 ...
                                 double kp, double kd)
    : FSMState(FSMStateName::FIXEDSTAND, "FIXEDSTAND", ctrl_interfaces),
      target_pos_(stand_pos), kp_(kp), kd_(kd), loop_count_(0) {}

void StateFixedStand::enter() { loop_count_ = 0; }

void StateFixedStand::run(const rclcpp::Time & /*time*/,
